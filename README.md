...
Only modification is required in the `game_agent.py` file to complete the project.  Additional files include example Player and evaluation functions, the game board class, and a template to develop local unit tests.  


## Methodology

In order to complete the Isolation project, the code must passes all test cases for the required functions in `game_agent.py` and complete a report as specified in the rubric.

Following functions were implemented:

- `MinimaxPlayer.minimax()`: implemented minimax search
- `AlphaBetaPlayer.alphabeta()`: implemented minimax search with alpha-beta pruning
- `AlphaBetaPlayer.get_move()`: implemented iterative deepening search
- `custom_score()`: implemented own best position evaluation heuristic
- `custom_score_2()`: implemented own alternate position evaluation heuristic
- `custom_score_3()`: implemented own alternate position evaluation heuristic
